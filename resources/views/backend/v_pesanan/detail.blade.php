                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        ...
                        <table class="table table-bordered table-hover display">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td align="center"><img
                                            src="{{ asset('storage/imgproduk/thumb_sm_' . $item->produk->foto) }}"
                                            alt="" width="60%"></td>
                                    <td class="details">
                    ...
                    <div class="col-xs-6 col-sm-6 col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror">
                                <option value="" {{ old('status', $order->status) == '' ? 'selected' : '' }}>- Pilih
                                    Status Pesanan -</option>
                                <option value="Paid" {{ old('status', $order->status) == 'Paid' ? 'selected' : '' }}>
                                    Proses</option>
                                <option value="Kirim" {{ old('status', $order->status) == 'Kirim' ? 'selected' : '' }}>
                                    Kirim</option>
                                <option value="Selesai"
                                    {{ old('status', $order->status) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                            </select>
                            @error('status')
                            <span class="invalid-feedback alert-danger" role="alert">
                                {{ $message }}
                    ...
                    <div class="col-xs-6 col-sm-6 col-md-6">
                        <div class="form-group">
                            ...
                            <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror"
                                id="ckeditor">{{ old('alamat',$order->alamat) }}</textarea>
                            @error('alamat')
                            <span class="invalid-feedback alert-danger" role="alert">
                                {{ $message }}
                            </span>
